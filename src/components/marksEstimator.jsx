...
export function MarksEstimator(props){
    function displayGrade(subject){
        return calculateRequiredMark(subject.subjectInternalMarks, subject.subjectType).map((grade) =>{
            ...
            return <li key = {grade.gradeStatus} className = "flex items-center justify-between rounded-xl border border-slate-700/50 bg-slate-800/60 px-4 py-3 transition-all duration-200 hover:border-amber-400/40 hover:bg-slate-800/80">
                        <span className="flex items-center gap-3 text-slate-100 font-semibold">
                            <span className={`w-2.5 h-2.5 rounded-full shadow-md ${dotColor}`} ></span>
                            {grade.gradeStatus} 
                        </span>
                        <span className="text-slate-400 text-sm font-medium">{grade.possibility}</span>
                        <span className="rounded-md bg-amber-500/10 px-3 py-1 text-amber-400 font-semibold border border-amber-400/20">{grade.marksRequired}</span>
                    </li>
        });
    }
